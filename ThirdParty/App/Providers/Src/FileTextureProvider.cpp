...
Ptr<Texture> FileTextureProvider::LoadTexture(const char* filename, RenderDevice* device)
{
    ...
    if (n == 4)
    {
        ...
        for (int i = 0; i < x * y; i++)
        {
            ...
            stbi_uc a = sRGB ? mGammaTable[img[4 * i + 3]] : img[4 * i + 3];
            stbi_uc r = (stbi_uc)(((uint32_t)img[4 * i] * a) / 255);
            stbi_uc g = (stbi_uc)(((uint32_t)img[4 * i + 1] * a) / 255);
            stbi_uc b = (stbi_uc)(((uint32_t)img[4 * i + 2] * a) / 255);
